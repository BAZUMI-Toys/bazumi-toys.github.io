"""
BAZUMI — скачивание всех фото товаров с WB CDN в локальную папку photos/

Использование:
1. Положи этот файл рядом с index.html (в папку bazumi-toys.github.io)
2. Открой командную строку (cmd) в этой папке (правый клик в проводнике → Открыть в терминале)
3. Запусти: python download_photos.py
4. Жди ~5–10 минут пока скачаются все 215 фото
"""
import os, sys, time
...
# User-Agent с реальным браузером, чтобы WB не блокировал
HEADERS = {
...
print(f'Скачиваю {len(PHOTOS)} фото в папку {OUT_DIR}/...')
print('-' * 60)
...
for i, (wb_id, url) in enumerate(PHOTOS, 1):
    ...
    if os.path.exists(fname) and os.path.getsize(fname) > 1000:
        skipped += 1
        if i % 25 == 0:
            print(f'  [{i}/{len(PHOTOS)}] ok={ok} skip={skipped} fail={len(failed)}')
        continue
    try:
        ...
        failed.append((wb_id, str(e)))
    time.sleep(0.1)  # лёгкая пауза, чтобы не зафлудить CDN
    if i % 25 == 0:
        print(f'  [{i}/{len(PHOTOS)}] ok={ok} skip={skipped} fail={len(failed)}')
...
if failed:
    print('\nНе удалось скачать (артикул WB → ошибка):')
    for wb_id, err in failed[:30]:
        print(f'  {wb_id}: {err}')
    if len(failed) > 30:
        print(f'  ... и ещё {len(failed) - 30}')
    # сохраняем список неудач в файл
    with open('failed_photos.txt', 'w', encoding='utf-8') as f:
        for wb_id, err in failed:
            f.write(f'{wb_id}\t{err}\n')
    print('\nСписок неудач сохранён в failed_photos.txt')
